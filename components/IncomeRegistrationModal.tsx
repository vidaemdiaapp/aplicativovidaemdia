...
export const IncomeRegistrationModal: React.FC<Props> = ({ isOpen, onClose, onSuccess }) => {
    ...
    const loadInitialData = async () => {
        ...
        if (partner) {
            ...
            setPartnerIncome({
                amount: pIncome?.amount_monthly.toString() || '',
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Save partner's income if in household and modified
            if (partnerIncome.user_id) {
                await incomesService.upsertIncome({
                    user_id: partnerIncome.user_id,
                    amount_monthly: parseFloat(partnerIncome.amount) || 0,
                    is_shared: true // Partner income in household is by default shared here
                });
            }
