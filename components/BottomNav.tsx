...
export const BottomNav: React.FC = () => {
  ...
  const NavItem = ({ path, icon: Icon, label }: { path: string; icon: any; label: string }) => (
    <button 
      ...
      className={`flex flex-col items-center justify-center gap-1 w-full h-full ${
        isActive(path) ? 'text-blue-600' : 'text-slate-400 hover:text-slate-600'
      }`}
    >
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 pb-safe pt-2 px-2 h-[80px] z-50 max-w-md mx-auto">
      <div className="flex justify-between items-center h-full pb-2">
        ...
        <div className="relative -top-5">
          <button 
            ...
            className="bg-blue-600 text-white rounded-full p-4 shadow-lg shadow-blue-200 hover:bg-blue-700 transition-colors active:scale-95"
          >
